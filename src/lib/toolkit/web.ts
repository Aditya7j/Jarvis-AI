...
export async function findValidatedOfficeholderArticle(
  ...
  const canonicalTitle = `${titleCase(office)} of ${displayTitle(placeForSearch)}`;
...
function rankQualifiedCanonicalTitle(rq: KnowledgeQuery): string {
  ...
  return `${titleCase(office)} of ${displayTitle(placeForTitle)}`.trim();
}
...
async function findCapitalArticle(
  ...
  const placeForSearch = canonicalPlace ?? place;
  const canonicalTitle = displayTitle(placeForSearch);
...
async function searchKnowledgeFallback(
  ...
  if (cls.kind === "officeholder") {
    ...
    const officeDisplay =
      article.title.replace(/\s+of\s+.*$/i, "").trim() || titleCase(cls.office);
    const placeDisplayBase = displayTitle(cls.place);
